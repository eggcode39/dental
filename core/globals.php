<?php
/**
 * Created by PhpStorm
 * User: CESARJOSE39
 * Date: 18/11/2019
 * Time: 15:07
 */

//Variables globales de uso en todo el sistema
//Definicion de servidor del aplicativo
define('_SERVER_', 'https://guabba.com/dental/');
//Definicion de variables para conexion de base de datos
define('_SERVER_DB_', '192.185.25.45');
define('_DB_', 'guabba_dental');
define('_USER_DB_', 'guabba_testeo');
define('_PASSWORD_DB_', 'a123Fgsdf');
//Definicion de clave de desencriptacion
define('_FULL_KEY','ñlasopweir');
define('_VIEW_PATH_','app/view/');